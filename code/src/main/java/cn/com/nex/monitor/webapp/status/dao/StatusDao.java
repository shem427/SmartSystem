...
public class StatusDao {
    ...
    public StatusBean getUnitStatus(String unitId) {
        ...
        String normalSql = "SELECT COUNT(1) AS NORMAL FROM `RADIATION` R WHERE R.`RAD_VALUE` >= ? AND R.`UNIT_ID` IN (SELECT `UNIT_ID` FROM `unit` U WHERE U.`LEAF`=true AND LEFT(getUnitPath(U.`UNIT_ID`), ?) = ?)";
        String warningSql = "SELECT COUNT(1) AS WARN FROM `RADIATION` R WHERE R.`RAD_VALUE` > ? AND R.`RAD_VALUE` < ? AND R.`UNIT_ID` IN (SELECT `UNIT_ID` FROM `unit` U WHERE U.`LEAF`=true AND LEFT(getUnitPath(U.`UNIT_ID`), ?) = ?)";
        String errorSql = "SELECT COUNT(1) AS ERROR FROM `RADIATION` R WHERE R.`RAD_VALUE` <= ? AND R.`UNIT_ID` IN (SELECT `UNIT_ID` FROM `unit` U WHERE U.`LEAF`=true AND LEFT(getUnitPath(U.`UNIT_ID`), ?) = ?)";
        jdbcTemplate.query(normalSql, new Object[] { thresholdBean.getNorml(), unitPathLength, bean.getUnitPath() }, rs -> {
            bean.setNormalRadiationData(rs.getInt("NORMAL"));
        });
        jdbcTemplate.query(warningSql, new Object[] { thresholdBean.getWarn(), thresholdBean.getNorml(), unitPathLength, bean.getUnitPath() }, rs -> {
            bean.setWarningRadiationData(rs.getInt("WARN"));
        });
        jdbcTemplate.query(errorSql, new Object[] { thresholdBean.getWarn(), unitPathLength, bean.getUnitPath() }, rs -> {
